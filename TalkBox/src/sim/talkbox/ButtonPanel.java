...
public class ButtonPanel extends JPanel {
	...
	public ButtonPanel() {
		...
		getInfo = new TalkBoxDeserializer();
		// Get number of audio buttons from TalkBoxDeserializer

		int numOfButtons = getInfo.getNumberOfAudioButtons();

		// fields to help add correct amount of buttons with respect to size of JFrame
		// of simulator.
		int trackColumns = 0;
		int trackRows = 0;
		...
		if (numOfButtons < 19) {
			...
			for (int i = 0; i < numOfButtons; i++) {
				...
				if (20 + ((i + 1 - trackColumns) * 150) > TalkBoxSim.frameWidth
						&& (80 + trackRows) < TalkBoxSim.frameHeight) {
					trackColumns += 6;
					trackRows += 80;
				}

				if (20 + ((i + 1 - trackColumns) * 150) < TalkBoxSim.frameWidth
						&& (80 + trackRows) < TalkBoxSim.frameHeight) { // +1 to account for checking LAST Jbutton that
																		// fits in Frame
			...
			for (int i = 0; i < numOfButtons; i++) {
				/*
				 * check if buttons will fit in frame going left to right if not go to next row
				 * in frame i -11 each row b/c 11 buttons per row.
				 */
				if (20 + ((i + 1 - trackColumns) * 80) > TalkBoxSim.frameWidth
						&& (80 + trackRows) < TalkBoxSim.frameHeight) {
					trackColumns += 11;
					trackRows += 40;
				}
